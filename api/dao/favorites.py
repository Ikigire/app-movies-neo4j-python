   ...
   *
    ...
    def add(self, user_id, movie_id):
        ...
        def add_to_favorites(tx, user_id, movie_id):
            ...
            # If no rows affecteds then raise a new exception
            if row is None:
        ...
        with self.driver.session() as session:
            return session.execute_write(add_to_favorites, user_id=user_id, movie_id=movie_id)
    # end::add[]
    ...
    def remove(self, user_id, movie_id):
        # Define a new transaction function to create a HAS_FAVORITE relationship
        def remove_favorites(tx, user_id, movie_id):
            row = tx.run("""
        ...
        with self.driver.session() as session:
            return session.execute_write(remove_favorites, user_id=user_id, movie_id=movie_id)
    # end::remove[]
