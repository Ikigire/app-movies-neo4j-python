...
class MovieDAO:
    ...
    # Define the Unit of Work
        def get_movies(tx, sort, order, limit, skip, user_id):
            # Getting the favorite movies of the user
            favorites = self.get_user_favorites(tx=tx, user_id=user_id)
    ...
    def find_by_id(self, id, user_id=None):
        ...
        def find_movie_by_id(tx, id, user_id = None):
            ...
            RETURN m {
                .*,
                actors: [ (a)-[r:ACTED_IN]->(m) | a { .tmdbId, .imdbId, .name, .bio, .poster, .url, role: r.role } ],
                directors: [ (d)-[:DIRECTED]->(m) | d { .tmdbId, .imdbId, .name, .bio, .poster, .url } ],
